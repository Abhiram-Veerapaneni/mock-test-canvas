  ...
  return (
    <div className="min-h-screen bg-[#edf2f9] dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 flex flex-col transition-colors duration-200">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-7">
        ...
        <section className="rounded-2xl bg-white dark:bg-[#1e293b] border border-slate-200/90 dark:border-[#334155] p-6 sm:p-7 shadow-xs">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
            ...
            <div className="flex items-center gap-3">
              <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
                <div className="bg-slate-50/80 dark:bg-[#151f32] p-3 rounded-xl border border-slate-200/80 dark:border-[#334155] text-center min-w-[84px]">
          ...
          {/* ── Table Switch Tabs ─────────────────────────────────────────── */}
          <div className="flex items-center gap-2 mt-6 pt-5 border-t border-slate-100 dark:border-[#334155]">
            <button
        ...
        {error && (
          <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900 text-rose-700 dark:text-rose-300 text-xs flex items-center justify-between">
            ...
            <button
              onClick={fetchData}
              className="px-3 py-1 bg-white dark:bg-[#1e293b] border border-rose-300 dark:border-rose-800 rounded-lg text-xs font-medium"
            >
              Retry
            </button>
          </div>
        ...
        {!isLoading && !error && (
          <>
            {/* ═════════════════════════════════════════════════════════════════════
               ...
               ═════════════════════════════════════════════════════════════════════ */}
            {activeTab === 'violations' && (
              <section className="bg-white dark:bg-[#1e293b] border border-slate-200/90 dark:border-[#334155] rounded-2xl shadow-xs overflow-hidden space-y-5 p-6">
                {/* Filters */}
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                          violationTypeFilter === type
                            ...
                            : 'bg-slate-100 dark:bg-[#243147] text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-200/80 dark:border-[#334155]'
                        }`}
                      >
                    ...
                    <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-[#243147] text-slate-700 dark:text-slate-300 font-medium cursor-pointer ml-1 select-none border border-slate-200/80 dark:border-[#334155]">
                      <input
                        type="checkbox"
                  ...
                  <div className="relative w-full sm:w-64">
                    ...
                    <input
                      ...
                      className="input-base text-xs pl-8 pr-3 py-1.5"
                    />
                  </div>
                </div>
                ...
                {filteredViolations.length === 0 ? (
                  ...
                  <div className="overflow-x-auto rounded-xl border border-slate-200/80 dark:border-[#334155]">
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50/80 dark:bg-[#151f32] border-b border-slate-200/80 dark:border-[#334155] text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-[#334155]">
                        {filteredViolations.map((v, idx) => (
                          <tr
                            key={idx}
                            className="hover:bg-slate-50/80 dark:hover:bg-[#243147]/50 transition-colors"
                          >
                            {/* 1. Timestamp */}
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <select
                      ...
                      className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-[#151f32] text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-[#334155] focus:outline-none cursor-pointer"
                    >
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-[#334155]">
                        {filteredCompletion.map((item, idx) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-4 whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-full bg-slate-200 dark:bg-[#243147] text-slate-700 dark:text-slate-300 font-bold flex items-center justify-center text-xs">
        ...
        {selectedSnapshot && (
          ...
          >
            <div
              className="max-w-2xl w-full bg-white dark:bg-[#1e293b] border border-slate-200/90 dark:border-[#334155] rounded-2xl p-6 text-slate-900 dark:text-slate-100 space-y-4 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header with Close Button */}
              <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-[#334155] pb-3">
                <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setSelectedSnapshot(null)}
                  className="p-1.5 rounded-lg btn-secondary text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
                  title="Close Modal"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              {/* Image Container */}
              <div className="rounded-xl overflow-hidden bg-slate-100 dark:bg-[#0f172a] border border-slate-200 dark:border-[#334155] max-h-[60vh] flex items-center justify-center">
                <img
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => setSelectedSnapshot(null)}
                    className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-[#243147] dark:hover:bg-[#2e3e5b] text-slate-700 dark:text-slate-300 text-xs font-medium cursor-pointer transition-colors border border-slate-200 dark:border-[#334155]"
                  >
                    Close
                  </button>
                </div>
